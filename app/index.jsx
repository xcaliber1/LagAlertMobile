...
if (!result.canceled) {
  ...
  const [dataNotifcation , setDataNotification] = useState([]);
  ...
  const handleEmergencyPress = async () => {
    try {
      ...
      if (findEmail) {
        setDataNotification(findEmail); // Update state with found report
        setVerificationStep(3); // Proceed to step 3
      } else {
        Alert.alert("No Reports", "No reports found for the given email.");
      }
    } catch (error) {
  ...
  const sendEmergencyData = async () => {
    try {
      ...
      if (geocodeResponse.status === 200 && geocodeResponse.data) {
        locationName = geocodeResponse.data.display_name || locationName;
      }
      ...
      await set(newEmergencyRef, {
        ...
        isVideo,
        location: {
          name: locationName, // Store location name
          latitude,
          longitude,
        },
  ...
  useEffect(() => {
    ...
    const fetchReports = async () => {
      try {
        ...
        if (findEmail) {
          setDataNotification(findEmail);
          setVerificationStep(3);
          clearInterval(intervalId); // Stop polling when data is found
        }
      } catch (error) {
        console.error("Error fetching reports:", error);
      }
  ...
  if (buttonText === "Waiting for Response") {
    if (verificationStep === 3 && dataNotifcation?.message) {
      // Show the notification screen when data is available
      return (
        <View style={styles.responseContainer}>
          <FontAwesome5 name="bell" size={100} color="gray" style={styles.iconStyle} />
          <Text style={styles.responseMessageTitle}>A message from your Responders</Text>
          <Text style={styles.responseMessage}>{dataNotifcation.message}</Text>
          <Text style={styles.responseFrom}>
            <Text style={styles.responseFromBold}>-From</Text> Admin
          </Text>
          <TouchableOpacity
            onPress={() => {
              setVerificationStep(4); // Reset to initial step
              setButtonText("Emergency"); // Reset the button
              setDataNotification(null); // Clear notification
            }}
            style={styles.markSafeButton}
          >
            <Text style={styles.markSafeButtonText}>Mark myself safe</Text>
          </TouchableOpacity>
        </View>
      );
    } else {
      // Stay on "Waiting for Response" if no data is found
      return (
        <View style={styles.container}>
